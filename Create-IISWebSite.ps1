...
function Add-SiteDNS {
    ...
    #$externalSiteAddress = (Resolve-DnsName "www.$($ConfigObject.DefaultDomain)").IPAddress
    ...
    if ($newSiteAddress -eq $null) {
        # No record exsists so create it
        Add-DnsServerResourceRecord -CName -Name $HostName -HostNameAlias $HostServer -ZoneName $ConfigObject.DefaultDomain -ComputerName $dnsServer | Out-Null
        Write-Host "DNS record for $HostName created" 
    }
    else {
        Write-Host "DNS record for $HostName already exists"
...
try {
    ...
    if (-not $HostHeader.endswith('aardman.com')) {
        $HostHeader = "$HostHeader.aardman.com"
    }
